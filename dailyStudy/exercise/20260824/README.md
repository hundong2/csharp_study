# 2026-08-24: 개인정보 내보내기 계획

## 처음 읽는 순서

1. 아래 **기초 문법 지도**를 읽습니다.
2. [`Program.cs`](./src/DataExportExercise/Program.cs)를 `조립 → 실행 → 모델 → 계약 → 서비스 → 구현 → 테스트` 순서로 읽습니다.
...
```powershell
cd dailyStudy/exercise/20260824/src/DataExportExercise
dotnet build
dotnet run
...
- `bool`은 참/거짓, `int`는 정수, `enum`은 제한된 이름 목록, `DateTimeOffset`은 시간대가 포함된 시각입니다.
...
흐름은 `Program`(Composition Root) → `PlanExportsService`(Application Service) → `IExportPolicy`(Strategy)와 Repository입니다. 생성자 주입은 인터페이스에 의존하는 DI/DIP이며 테스트 대역 교체를 쉽게 합니다.

`ExportRequest`와 `ExportPlan`은 Domain Model입니다. record와 읽기 전용 목록은 처리 중 값이 뜻밖에 바뀌는 일을 줄입니다. Application Service는 사용 사례 순서만 담당해 SRP를 지키고, 새 정책은 Strategy 구현 추가로 확장해 OCP를 지킵니다. 이는 SOLID를 작은 코드에 적용한 예입니다.
...
실무에서는 내보내기 생성과 알림이 다른 시스템일 수 있습니다. 요청 ID 고유 제약과 멱등 키로 중복 생성을 막고, 낙관적 동시성으로 본인 확인 상태 변경과의 경합을 검출합니다. DB 변경과 작업 메시지를 함께 보장하려면 Outbox를 고려합니다. 파일은 암호화하고 짧은 보존기간 후 삭제하며, 서명 URL 만료·접근 감사·용량 제한을 둡니다. `CancellationToken`을 끝까지 전달하고 개인정보는 로그에서 제외하며 처리량, 실패율, 생성 지연, 삭제 실패를 메트릭과 분산 추적으로 관찰합니다.

## 버전 업데이트 (2026-08-24 확인)
...
- 최신 미리보기는 **.NET 11 Preview 7 / C# 15**입니다. C# 15의 union types, closed hierarchies, extension indexers, collection-expression arguments, labeled `break`/`continue` 등은 안정 실행 코드에서 제외했습니다. 미리보기 기능은 별도 실험 프로젝트에서만 평가하세요.
...
- [ ] 멱등성, 동시성, Outbox, 보존기간, 취소, 관측성, 개인정보 로그 금지 이유를 말한다.
