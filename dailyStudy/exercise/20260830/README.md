# 2026-08-30: 반품 요청 처리 계획

## 처음 읽는 순서

1. 아래 **기초 문법 지도**를 읽습니다.
2. [`Program.cs`](./src/ReturnRequestExercise/Program.cs)를 `실행 → 모델 → 계약 → 서비스 → 구현 → 테스트` 순서로 읽습니다.
...
```powershell
cd dailyStudy/exercise/20260830/src/ReturnRequestExercise
dotnet build
dotnet run
...
- `decimal`은 금액, `int`는 정수, `bool`은 참/거짓, `enum`은 제한된 이름 목록입니다. `39_000m`의 `_`는 읽기 구분자이고 `m`은 decimal 리터럴 표시입니다.
...
흐름은 `Program`(Composition Root) → `PlanReturnsService`(Application Service) → `IReturnPolicy`(Strategy)와 Repository입니다. 생성자 주입은 인터페이스에 의존하는 DI/DIP이며 테스트 대역 교체를 쉽게 합니다.

`ReturnRequest`와 `ReturnPlan`은 Domain Model입니다. record와 읽기 전용 목록은 처리 중 값이 뜻밖에 바뀌는 일을 줄입니다. Application Service는 사용 사례 순서만 맡아 SRP를 지키고, 정책 구현 추가로 기준을 확장해 OCP를 지킵니다. 이는 SOLID를 작은 코드에 적용한 예입니다.
...
실무에서는 반품 요청 ID와 버전으로 멱등 키를 만들고 `ExpectedVersion` 낙관적 동시성으로 상담원과 자동 처리의 경합을 검출합니다. 결과 저장과 환불·물류 이벤트 발행을 함께 보장하려면 트랜잭션과 Outbox를 고려합니다. `CancellationToken`을 DB까지 전달하고 승인률·수동 검토율·처리 지연·저장 충돌률을 메트릭과 분산 추적으로 관찰합니다. 주문·고객·주소 정보는 최소 수집, 암호화, 접근 통제, 보존·삭제 정책을 적용하고 원문을 로그에 남기지 않습니다. 실제 반품 정책은 국가별 소비자법과 상품군 규칙을 정책 버전으로 관리하고 결정 근거를 감사 가능하게 보존해야 합니다.

## 버전 업데이트 (2026-08-30 확인)
